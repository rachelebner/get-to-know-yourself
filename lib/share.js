...
/**
 * Apply markdown-style bold (**text**) and italic (*text*) to a string, then escape HTML.
 * Bold is applied first so ** is not treated as two italics.
 */
function formatInlineUnsafe(s) {
  return s
...
/**
 * Convert markdown to HTML so rich-text paste shows formatting, not raw markdown.
...
export function markdownToHtml(markdown) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^###\s/.test(line)) {
      ...
      out.push('<h3>', formatInlineUnsafe(trimmed.slice(3).trim()), '</h3>');
      continue;
    }
    if (/^##\s/.test(line)) {
      if (inList) { out.push('</ul>'); inList = false; }
      out.push('<h2>', formatInlineUnsafe(trimmed.slice(2).trim()), '</h2>');
      continue;
    }
    if (/^#\s/.test(line)) {
      if (inList) { out.push('</ul>'); inList = false; }
      out.push('<h1>', formatInlineUnsafe(trimmed.slice(1).trim()), '</h1>');
      continue;
    }
    if (/^[-*]\s+/.test(trimmed)) {
      if (!inList) { out.push('<ul>'); inList = true; }
      const content = trimmed.replace(/^[-*]\s+/, '');
      out.push('<li>', formatInlineUnsafe(content), '</li>');
      continue;
    }
    if (inList) { out.push('</ul>'); inList = false; }
    if (trimmed === '') {
    ...
    out.push('<p>', formatInlineUnsafe(trimmed), '</p>');
  }
...
/**
 * Copy markdown as rich text by converting to HTML first. Use this so pasted content
 * is formatted (headings, bold, lists) instead of raw markdown.
 * @param {string} markdown - Markdown text
 * @returns {Promise<void>}
 */
export async function copyAsRichTextFromMarkdown(markdown) {
  return copyAsRichText(markdown, { fromMarkdown: true });
}

/**
 * Copy as rich text (HTML) - pastes formatted in Word, Google Docs, etc.
 * If content is markdown, pass { fromMarkdown: true } to convert to HTML first.
 * @param {string} content - HTML string, or markdown when fromMarkdown is true
 * @param {{ fromMarkdown?: boolean }} [options] - Set fromMarkdown: true to convert markdown to HTML before copying
 * @returns {Promise<void>}
 */
export async function copyAsRichText(content, options = {}) {
  const html = options.fromMarkdown === true ? markdownToHtml(content) : content;
  try {
